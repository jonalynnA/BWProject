...
class MealViewController: UIViewController, UITextFieldDelegate,
  UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let rating = ratingControl.rating ?? 1
        ...
        // unwind segue.
       let meal = Meal(restaurantName: restaurantName, location: location, hours: hours, foodieReview: foodieReview, foodOrdered: foodOrdered, rating: rating)
        mealTest.put(meal: meal) {error in
            if let error = error {
                print(error)
            } else {
                print("success")
